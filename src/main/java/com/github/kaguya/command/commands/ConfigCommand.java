    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            ChatUtil.sendFormatted(
                    String.format("%sUsage: .%s &oload&r/&osave&r <&oname&r> | .%s &olist&r | .%s &ofolder&r", Kaguya.clientName, command, command, command)
            );
        } else {
            ...
            switch (sub) {
                ...
                case "reload":
                    if (args.size() < 3) {
                        ChatUtil.sendFormatted(
                                String.format("%sMissing config name (use '&odefault&r' or '&o!&r' to load default config)&r", Kaguya.clientName)
                        );
                        return;
                    }
                ...
                case "list":
                    try {
                        ...
                        if (configs.length == 0) {
                            ChatUtil.sendFormatted(String.format("%sNo configs found (&o%s&r)&r", Kaguya.clientName, "./config/Myau/"));
                        }
                        Arrays.sort(configs, LastModifiedFileComparator.LASTMODIFIED_REVERSE);
                        ChatUtil.sendFormatted(String.format("%sConfigs:&r", Kaguya.clientName));
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to read (&o%s&r)&r", Kaguya.clientName, "./config/Myau/"));
                    }
                    return;
                case "f":
                case "folder":
                ...
                case "directory":
                    try {
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to open (&o%s&r)&r", Kaguya.clientName, "./config/Myau/"));
                    }
                    return;
                default:
                    ChatUtil.sendFormatted(String.format("%sInvalid argument (&o%s&r)&r", Kaguya.clientName, args.get(1)));
            }
        }
    }
}
